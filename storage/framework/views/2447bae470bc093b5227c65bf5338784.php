...
<div class="admin-table-toolbar mb-3">
    <div class="d-flex flex-wrap justify-content-between align-items-end gap-2">
        <form class="d-flex flex-wrap align-items-end gap-2 flex-grow-1" method="GET">
            <?php $__currentLoopData = request()->only(['type','placement','section_key','row_title']); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $hiddenKey => $hiddenValue): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php if($hiddenValue !== null && $hiddenValue !== ''): ?>
                    <input type="hidden" name="<?php echo e($hiddenKey); ?>" value="<?php echo e($hiddenValue); ?>">
                <?php endif; ?>
        ...
        <div class="d-flex flex-wrap justify-content-end align-items-end gap-2 admin-toolbar-actions">
            ...
            <form method="POST" action="<?php echo e(route('admin.common-import.store', ['module' => $module['key']])); ?>" enctype="multipart/form-data" class="d-inline admin-import-form">
                <?php echo csrf_field(); ?>
</form>
            <button class="btn btn-outline-primary admin-toolbar-icon" type="button" data-bs-toggle="modal" data-bs-target="#importModal<?php echo e($module['key']); ?>" title="Import Excel/CSV" aria-label="Import Excel/CSV">
                <i class="fa-solid fa-file-import"></i><span class="d-none d-xl-inline ms-1">Import</span>
            </button>

            <div class="modal fade" id="importModal<?php echo e($module['key']); ?>" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Import <?php echo e($module['label'] ?? $pageTitle); ?></h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>

                        <form method="POST" action="<?php echo e(route('admin.common-import.store', ['module' => $module['key']])); ?>" enctype="multipart/form-data">
                            <?php echo csrf_field(); ?>
                            <?php $__currentLoopData = request()->only(['type','placement','section_key','row_title']); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $hiddenKey => $hiddenValue): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php if($hiddenValue !== null && $hiddenValue !== ''): ?>
                                    <input type="hidden" name="<?php echo e($hiddenKey); ?>" value="<?php echo e($hiddenValue); ?>">
                            ...
                            <div class="modal-body">
                                <div class="alert alert-info small mb-3">
                                    Download sample file, fill data, then upload here. This import affects only current submenu: <strong><?php echo e($module['label'] ?? $pageTitle); ?></strong>.
                                </div>

                                <a href="<?php echo e(route('admin.common-import.sample', ['module' => $module['key']])); ?>" class="btn btn-outline-success w-100 mb-3">
...
</div><?php /**PATH C:\All Project flutter laravel reactjs python\Bawaskar Technology\bawaskar_erp\resources\views\admin\shared\table-toolbar.blade.php ENDPATH**/ ?>